...
//2. change: move swinging mode to a different function and just start it untill you write 0 
void loopMode(int delaytime){
    ...
    if (!pos1Valid || !pos2Valid) {
      ...
      while (true) {
        ...
        Serial.println("moving to position number two");
        Serial.println(pos2);
...
void setup() {
  ...
  //1.change: start homing 
  home();
  //then start looping from A to B 
  loopMode(2000);
}
